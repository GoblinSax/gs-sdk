...
var GoblinSaxAPI = class {
  constructor(signer, apiKey, version) {
    ...
    this.nftfi_contract = new import_ethers.ethers.Contract(
      this.envConfig.nftfi,
      nftfi_default,
      this.signer
    );
    this.nftfi_note = new import_ethers.ethers.Contract(
      this.envConfig.nftfi_promissory_note,
      nftfiNoteReceipt_default,
      this.signer
    );
    this.nftfi_obligation = new import_ethers.ethers.Contract(
      this.envConfig.nftfi_obligation_receipt,
      nftfiNoteReceipt_default,
      this.signer
    );
    this.weth_contract = new import_ethers.ethers.Contract(
      this.envConfig.weth,
      erc20_default,
      this.signer
    );
    this.bnpl_contract = new import_ethers.ethers.Contract(
      this.envConfig.bnpl,
      bnpl_default,
      this.signer
    );
  ...
  async getLoans(alchemyApiKey) {
    ...
    for (let asset of gsPromissoryNotesAssets) {
      ...
      if (loanInfo.borrower == import_ethers.ethers.constants.AddressZero && signerObligationReceiptAssets.find(
  ...
  async checkApprovedWETH() {
    ...
    if (x.lt(import_ethers.BigNumber.from(10).pow(18).mul(1e3))) {
      return false;
    } else {
      return true;
    }
  ...
  async approveSpendingWETH() {
    return this.weth_contract.approve(
      this.envConfig.nftfi,
      import_ethers.ethers.constants.MaxUint256
    );
  }
  async checkApprovedNFT(collection) {
    try {
      let erc721_contract = new import_ethers.ethers.Contract(
        collection,
        erc721_default,
        this.signer
      );
  ...
  async createOffer(collection, assetId, duration, borrowerAddress, principal, apr) {
    ...
    return {
      ...
      serviceFee: {
        fee: import_ethers.BigNumber.from(loan.result.service_fee.service_fee.toString()),
  ...
  async getOSListing(collection, assetId) {
    ...
    let osRes = await import_axios.default.get(osApiUrl);
  ...
  async bnplAllowance(token, marketPrice, principal, gsFee) {
    ...
    const requiredAllowance = import_ethers.BigNumber.from(marketPrice).sub(principal).add(gsFee);
    return {
  ...
  async bnplOS(collection, assetId, duration, borrowerAddress, principal, apr) {
    let iface = new import_ethers.ethers.utils.Interface(seaport_default);
    ...
    const fulfillAdvancedOrder = () => {
      const advancedOrder = {
        ...
        extraData: import_ethers.ethers.constants.HashZero
      };
      try {
        return iface.encodeFunctionData("fulfillAdvancedOrder", [
          advancedOrder,
          [],
          import_ethers.ethers.constants.HashZero,
          import_ethers.ethers.constants.AddressZero
        ]);
      } catch (error) {
        console.log(error);
        throw error;
    ...
    const executeParams = {
      ...
      assetType: osOffer.itemType == 2 ? import_ethers.ethers.utils.formatBytes32String("ERC721") : import_ethers.ethers.utils.formatBytes32String("ERC1155"),
      buyData: fulfillAdvancedOrder(),
      totalPrice: import_ethers.BigNumber.from(listing.current_price).div(osOffer.endAmount),
      ...
      borrowerSettings: {
        revenueSharePartner: import_ethers.ethers.constants.AddressZero,
        referralFeeInBasisPoints: 0
      }
    };
    return this.bnpl_contract.execute(executeParams);
  }
  async executeBnpl(collection, assetId, marketPrice, assetType, duration, borrowerAddress, principal, apr, buyData, module2) {
    let gsOffer;
    try {
      gsOffer = await this.createOffer(
        collection,
    ...
    const executeParams = {
      module: module2,
      assetType: import_ethers.ethers.utils.formatBytes32String(assetType),
      buyData,
